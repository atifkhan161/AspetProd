...
{
    public partial class OtherItems : System.Web.UI.Page
    {
        ...
        {
            AspProdNet.Modals.MaterialOther oEntry = JsonConvert.DeserializeObject<AspProdNet.Modals.MaterialOther>(sData);

            using (var dbContext = new aspet_devEntities())
            {
                oEntry.wDateTime = DateTime.Now;
                oEntry.Status = 0;
                dbContext.MaterialOthers.Add(oEntry);
        ...
        public static string LoadSingleData(string sId)
        {
            ...
            using (var dbContext = new aspet_devEntities())
            {
                var oList = dbContext.MaterialOthers.Where(obj => obj.wOtherId == id).SingleOrDefault();
                sReturn = JsonConvert.SerializeObject(oList);
            }
            return sReturn;
        }
